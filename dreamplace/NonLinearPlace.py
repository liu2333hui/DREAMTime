...
class NonLinearPlace(BasicPlace.BasicPlace):
    ...
    def __call__(self, params, placedb):
        ...
        if params.global_place_flag:
            ...
            for global_place_params in params.global_place_stages:
                ...
                def one_descent_step(
                    ...
                    #write out the def for static timing analysis
                    if params.plot_flag and (iteration % 5 == 0 or iteration == 999):
                        logging.info("写DEF为了STA")
                        #cur_pos = self.pos[0].data.clone().cpu().numpy()
                        #self.plot(params, placedb, iteration, cur_pos)
                        placedb.write(params, "def/soln"+str(iteration) +".def")
        ...
        print(params.dump_global_place_solution_flag)
        print("!!!Dump global placement solution")
